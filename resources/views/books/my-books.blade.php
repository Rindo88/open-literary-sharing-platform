        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <h2 class="text-xl font-semibold text-gray-800 mb-4 flex items-center">
                    ...
                    Sedang Dibaca
                    <span
                        class="ml-2 bg-blue-100 text-blue-800 text-sm px-2 py-1 rounded-full">{{ $readingBooks->count() }}</span>
                </h2>
                        ...
                        @foreach ($readingBooks as $userBook)
                            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                                <div class="p-6">
                                    ...
                                    <p class="text-sm text-gray-500 mb-2">Dimulai:
                                        {{ $userBook->started_at ? $userBook->started_at->format('d M Y') : 'N/A' }}</p>
                                    <p class="text-sm text-blue-600 mb-4">📚 Status: Sedang dibaca</p>

                                    <div class="flex space-x-2">
                                        <a href="{{ route('books.show', ['type' => 'books', 'id' => $userBook->book->id]) }}"
                                        ...
                                        <form action="{{ route('books.show', ['type' => 'books', 'id' => $userBook->book->id]) }}}"
                ...
                @else
                    <div class="bg-white rounded-lg p-8 text-center">
                        ...
                        <a href="{{ route('books.index') }}"
                            class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 transition-colors min-h-[40px]">
                            Jelajahi Buku
                        </a>
                    </div>
                @endif
            ...
            <div class="mb-8">
                <h2 class="text-xl font-semibold text-gray-800 mb-4 flex items-center">
                    ...
                    Buku Selesai Dibaca
                    <span
                        class="ml-2 bg-green-100 text-green-800 text-sm px-2 py-1 rounded-full">{{ $finishedBooks->count() }}</span>
                </h2>
                        ...
                        @foreach ($finishedBooks as $userBook)
                            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                                <div class="p-6">
                                    <h3 class="text-lg font-semibold text-gray-900 mb-2">{{ $userBook->book->title }}</h3>
                                    <p class="text-gray-600 mb-2">oleh {{ $userBook->book->author }}</p>
                                    <p class="text-sm text-gray-500 mb-4">Selesai:
                                        {{ $userBook->finished_at ? $userBook->finished_at->format('d M Y') : 'N/A' }}</p>
            ...
            <div class="mb-8">
                <h2 class="text-xl font-semibold text-gray-800 mb-4 flex items-center">
                    ...
                    Wishlist
                    <span
                        class="ml-2 bg-yellow-100 text-yellow-800 text-sm px-2 py-1 rounded-full">{{ $wishlistBooks->count() }}</span>
                </h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach ($wishlistBooks as $userBook)
                            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                                <div class="p-6">
                                    <h3 class="text-lg font-semibold text-gray-900 mb-2">{{ $userBook->book->title }}</h3>
                                    <p class="text-gray-600 mb-2">oleh {{ $userBook->book->author->pen_name }}</p>
                                    <p class="text-sm text-gray-500 mb-4">Ditambahkan:
                                        {{ $userBook->created_at->format('d M Y') }}</p>
