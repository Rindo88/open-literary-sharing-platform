        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-gray-500 md:ml-2 text-sm sm:text-base truncate max-w-[150px] sm:max-w-none">{{ $book->title }}</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-lg shadow p-4 sm:p-6">
                ...
                <div class="mb-4 sm:mb-6">
                    ...
                    @else
                        <div class="w-full h-48 sm:h-64 lg:h-96 flex items-center justify-center bg-gray-200 rounded-lg">
                            <svg class="w-12 h-12 sm:w-16 sm:h-16 lg:w-24 lg:h-24 text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                ...
                @else
                    <div class="text-center py-3 sm:py-4">
                        ...
                        <a href="{{ route('login') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 sm:py-3 px-3 sm:px-4 rounded-lg inline-block text-sm sm:text-base">
                            Login
                        </a>
                    </div>
                @endauth
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-lg shadow p-4 sm:p-6 mb-4 sm:mb-6">
                <div class="mb-4">
                    <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-2 leading-tight">{{ $book->title }}</h1>
                    <p class="text-lg sm:text-xl text-gray-600 mb-3">oleh {{ $book->author }}</p>
                    <div class="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4">
                        <span class="inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium bg-blue-100 text-blue-800 w-fit">
                            {{ $book->category->name }}
                        </span>
                        @if($book->ratings->count() > 0)
                            <div class="flex items-center">
                                ...
                                <span class="ml-1 text-gray-600 text-sm sm:text-base">({{ $book->ratings->count() }} rating)</span>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="mt-4 sm:mt-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                        ...
                        <div class="bg-gray-50 p-3 rounded-lg">
                            ...
                            <p class="text-gray-600 text-sm sm:text-base">{{ $book->published_year ?? 'Tidak tersedia' }}</p>
                        </div>
                        <div class="bg-gray-50 p-3 rounded-lg">
                            <h4 class="font-semibold text-gray-900 text-sm sm:text-base mb-1">Penerbit</h4>
                            <p class="text-gray-600 text-sm sm:text-base">{{ $book->publisher ?? 'Tidak tersedia' }}</p>
                        </div>
                        <div class="bg-gray-50 p-3 rounded-lg">
                    ...
                    <div class="mb-4 sm:mb-6 p-3 sm:p-4 bg-gray-50 rounded-lg">
                        <h4 class="font-medium text-gray-900 mb-2 sm:mb-3 text-sm sm:text-base">Berikan Rating Anda</h4>
                            ...
                            <div>
                                ...
                                <div id="rating-stars" class="flex justify-center sm:justify-start space-x-1 sm:space-x-2">
                                    @for($i = 1; $i <= 5; $i++)
                            ...
                            <div>
                                <label for="review" class="block text-sm font-medium text-gray-700 mb-2">Review (Opsional)</label>
                            ...
                            <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-3 sm:px-4 rounded text-sm sm:text-base">
                ...
                @if($book->ratings->count() > 0)
                    <div class="space-y-3 sm:space-y-4">
                        @foreach($book->ratings as $rating)
                            <div class="border-b border-gray-200 pb-3 sm:pb-4 last:border-b-0 bg-gray-50 sm:bg-transparent p-3 sm:p-0 rounded-lg sm:rounded-none">
                                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2 space-y-1 sm:space-y-0">
                                    <div class="flex items-center space-x-2">
                                        <span class="font-medium text-gray-900 text-sm sm:text-base">{{ $rating->user->name }}</span>
                                        <div class="flex">
                                            @for($i = 1; $i <= 5; $i++)
                                    ...
                                    <span class="text-xs sm:text-sm text-gray-500 bg-white sm:bg-transparent px-2 py-1 sm:px-0 sm:py-0 rounded text-center sm:text-left">{{ $rating->created_at->diffForHumans() }}</span>
                                </div>
                                @if($rating->review)
                                    <p class="text-gray-700 text-sm sm:text-base bg-white p-2 rounded border border-gray-200 sm:bg-transparent sm:p-0 sm:border-0">{{ $rating->review }}</p>
                                @endif
                            </div>
                        @endforeach
                    </div>
                @else
                    <p class="text-gray-500 text-center py-6 sm:py-8 text-sm sm:text-base">Belum ada rating untuk buku ini.</p>
                @endif
            </div>
        </div>
    </div>
